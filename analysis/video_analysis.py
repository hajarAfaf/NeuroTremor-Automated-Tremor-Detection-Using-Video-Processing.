...
def analyze_video(video_path, patient_age=60, result_dir="static/results"):
    ...
    # def classify_tremor(features):
    #     amplitude = features[0]
    #     frequency = features[4]

    #     if amplitude < 0.001:
    #         return "Normal"
    #     elif 3 <= frequency <= 6 and amplitude > 0.002:
    #         return "Hyperkinetique"
    #     elif frequency < 3:
    #         return "Hypokinetique"
    #     elif frequency > 6:
    #         return "Ataxique"
    #     else:
    #         return "Indéterminé"
    # tremor_type = classify_tremor(features)
    ...
    graph_path = os.path.join(
        result_dir, f"signal_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    )
